...
var debug = require('debug')('ssp:paths');
var p     = require('path');
var fs    = require('fs')
...
function getDefaultPM2Home() {
  ...
  if (process.env.PM2_HOME)
    ...
    PM2_ROOT_PATH = p.resolve(process.env.HOME, '.ssp');
  else if (process.env.HOME || process.env.HOMEPATH)
    PM2_ROOT_PATH = p.resolve(process.env.HOMEDRIVE, process.env.HOME || process.env.HOMEPATH, '.ssp');
  else {
    console.error('[PM2][Initialization] Environment variable HOME (Linux) or HOMEPATH (Windows) are not set!');
    console.error('[PM2][Initialization] Defaulting to /etc/.ssp');
    PM2_ROOT_PATH = p.resolve('/etc', '.ssp');
  }

  debug('ssp home resolved to %s', PM2_ROOT_PATH, process.env.HOME);
...
module.exports = function(PM2_HOME) {
  ...
  var ssp_file_stucture = {
    ...
    PM2_LOG_FILE_PATH        : p.resolve(PM2_HOME, 'ssp.log'),
    PM2_PID_FILE_PATH        : p.resolve(PM2_HOME, 'ssp.pid'),
    ...
    PM2_IO_ACCESS_TOKEN      : p.resolve(PM2_HOME, 'ssp-io-token'),
    DUMP_FILE_PATH           : p.resolve(PM2_HOME, 'dump.ssp'),
    DUMP_BACKUP_FILE_PATH    : p.resolve(PM2_HOME, 'dump.ssp.bak'),
  ...
  if (process.platform === 'win32' ||
    ...
    ssp_file_stucture.DAEMON_RPC_PORT = '\\\\.\\pipe\\rpc.sock';
    ssp_file_stucture.DAEMON_PUB_PORT = '\\\\.\\pipe\\pub.sock';
    ssp_file_stucture.INTERACTOR_RPC_PORT = '\\\\.\\pipe\\interactor.sock';
  }

  return ssp_file_stucture;
};
