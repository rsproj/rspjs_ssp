...
var ssp_env = JSON.parse(process.env.ssp_env);
for(var k in ssp_env) {
  process.env[k] = ssp_env[k];
}

// Rename process
process.title = process.env.PROCESS_TITLE || 'node ' + ssp_env.pm_exec_path;

delete process.env.ssp_env;
...
(function ProcessContainer() {
  ...
  var stdFile     = ssp_env.pm_log_path;
  var outFile     = ssp_env.pm_out_log_path;
  var errFile     = ssp_env.pm_err_log_path;
  var pidFile     = ssp_env.pm_pid_path;
  var script      = ssp_env.pm_exec_path;
  ...
  if (cst.MODIFY_REQUIRE)
    require.main.filename = ssp_env.pm_exec_path;
  ...
  if (process.env.args != null)
    process.argv = process.argv.concat(ssp_env.args);
  ...
  // uid/gid management
  if (ssp_env.uid || ssp_env.gid) {
    try {
      if (process.env.gid)
        process.setgid(ssp_env.gid);
      if (ssp_env.uid)
        process.setuid(ssp_env.uid);
    } catch(e) {
      setTimeout(function() {
        console.error('%s on call %s', e.message, e.syscall);
        console.error('%s is not accessible', ssp_env.uid);
...
function exec(script, stds) {
  ...
  Utility.startLogging(stds, function (err) {
      ...
      return function(string, encoding, cb) {
        ...
        if (ssp_env.log_type && ssp_env.log_type === 'json') {
          log_data = JSON.stringify({
            ...
            timestamp : ssp_env.log_date_format && dayjs ?
              dayjs().format(ssp_env.log_date_format) : new Date().toISOString(),
            type : 'err',
            process_id : ssp_env.pm_id,
            app_name : ssp_env.name
          }) + '\n';
        }
        else if (ssp_env.log_date_format && dayjs)
          log_data = `${dayjs().format(ssp_env.log_date_format)}: ${string.toString()}`;
        else
    ...
    process.chdir(ssp_env.pm_cwd || process.env.PWD || p.dirname(script));
