...
const IOAPI = require('@ssp/js-api')
...
module.exports = class PM2ioHandler {
    ...
    switch (command) {
      ...
      case 'logout': {
        this._strategy.isAuthenticated().then(isConnected => {
          ...
          console.error(`${cst.PM2_IO_MSG_ERR} You can also contact us to get help: contact@ssp.io`)
        })
        break
      }
      case 'create': {
      ...
      case 'web': {
        this._strategy.isAuthenticated().then(res => {
          // if the user isn't authenticated, we make them do the whole flow
          if (res === false) {
            console.error(`${cst.PM2_IO_MSG_ERR} You need to be authenticated to do that, please use: ssp plus login`)
            return process.exit(1)
          }
      ...
      if (buckets.length === 0) {
        return this.createBucket((err, bucket) => {
          if (err) {
            ...
            if (bucket) {
              console.error(`${cst.PM2_IO_MSG_ERR} You can retry using: ssp plus link ${bucket.secret_id} ${bucket.public_id}`)
            }
            console.error(`${cst.PM2_IO_MSG_ERR} You can also contact us to get help: contact@ssp.io`)
            return process.exit(0)
          }
          const targetURL = `https://app.ssp.io/#/bucket/${bucket._id}`
  ...
  static validateAccount (token) {
    this.io.auth.validEmail(token)
      .then(res => {
        ...
        console.log(`${cst.PM2_IO_MSG} You can now proceed and use: ssp plus connect`)
        return process.exit(0)
      }).catch(err => {
        if (err.status === 401) {
  ...
  static createBucketHandler (err, bucket) {
    ...
    var targetURL = `https://app.ssp.io/#/bucket/${bucket._id}`
