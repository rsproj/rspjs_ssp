 ...
 *
 * ssp install http.tar.gz
 * ssp uninstall http
 *
 * - the first and only folder in the tarball must be called module (tar zcvf http module/)
 * - a package.json must be present with attribute "name", "version" and "ssp" to declare apps to run
 */
...
function runInstall(PM2, target_path, module_name, opts, cb) {
  ...
  if (opts.install) {
    ...
    sexec(`cd ${target_path} ; yarn install`, {silent: false}, function(code) {
      // Start apps under "apps" or "ssp" attribute
...
function publish(PM2, folder, cb) {
  ...
  if (!pkg.ssp && !pkg.apps) throw new Error('Attribute apps should be present')
  ...
  packager(current_path, target_path, (err, res) => {
    ...
    var uri = `${PM2.ssp_configuration.registry}/api/v1/modules`
