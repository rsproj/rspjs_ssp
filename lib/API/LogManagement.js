    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, list) {
      ...
      list.forEach(function(l) {
        if (typeof api == 'undefined') {
          ...
          Common.printOut(cst.PREFIX_MSG + l.ssp_env.pm_out_log_path);
          Common.printOut(cst.PREFIX_MSG + l.ssp_env.pm_err_log_path);

          if (l.ssp_env.pm_log_path) {
            Common.printOut(cst.PREFIX_MSG + l.ssp_env.pm_log_path);
            fs.closeSync(fs.openSync(l.ssp_env.pm_log_path, 'w'));
          }
          fs.closeSync(fs.openSync(l.ssp_env.pm_out_log_path, 'w'));
          fs.closeSync(fs.openSync(l.ssp_env.pm_err_log_path, 'w'));
        }
        else if (l.ssp_env.pm_id == api || l.ssp_env.name === api) {
          Common.printOut(cst.PREFIX_MSG + 'Flushing:');

          if (l.ssp_env.pm_log_path && fs.existsSync(l.ssp_env.pm_log_path)) {
            Common.printOut(cst.PREFIX_MSG + l.ssp_env.pm_log_path);
            fs.closeSync(fs.openSync(l.ssp_env.pm_log_path, 'w'));
          }

          if (l.ssp_env.pm_out_log_path && fs.existsSync(l.ssp_env.pm_out_log_path)) {
            Common.printOut(cst.PREFIX_MSG + l.ssp_env.pm_out_log_path);
            fs.closeSync(fs.openSync(l.ssp_env.pm_out_log_path, 'w'));
          }

          if (l.ssp_env.pm_err_log_path && fs.existsSync(l.ssp_env.pm_err_log_path)) {
            Common.printOut(cst.PREFIX_MSG + l.ssp_env.pm_err_log_path);
            fs.closeSync(fs.openSync(l.ssp_env.pm_err_log_path, 'w'));
          }
        }
  ...
  CLI.prototype.logrotate = function(opts, cb) {
    ...
    if (process.getuid() != 0) {
      return exec('whoami', function(err, stdout, stderr) {
        ...
        Common.printError(cst.PREFIX_MSG + chalk.grey('      sudo env PATH=$PATH:' + path.dirname(process.execPath) + ' ssp logrotate -u ' + stdout.trim()));
    ...
    try {
      fs.writeFileSync('/etc/logrotate.d/ssp-'+user, script);
    } catch (e) {
      console.error(e.stack || e);
    }

    Common.printOut(cst.PREFIX_MSG + 'Logrotate configuration added to /etc/logrotate.d/ssp');
  ...
  CLI.prototype.streamLogs = function(id, lines, raw, timestamp, exclusive, highlight) {
    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, list) {
      ...
      list.forEach(function(proc) {
        if (proc.ssp_env && (id === 'all' ||
                             proc.ssp_env.name == id ||
                             proc.ssp_env.pm_id == id)) {
          if (proc.ssp_env.pm_out_log_path && exclusive !== 'err')
            pushIfUnique({
              path     : proc.ssp_env.pm_out_log_path,
              app_name :proc.ssp_env.pm_id + '|' + proc.ssp_env.name,
              type     : 'out'});
          if (proc.ssp_env.pm_err_log_path && exclusive !== 'out')
            pushIfUnique({
              path     : proc.ssp_env.pm_err_log_path,
              app_name : proc.ssp_env.pm_id + '|' + proc.ssp_env.name,
              type     : 'err'
            });
        } else if(proc.ssp_env && proc.ssp_env.namespace == id) {
          if(namespaceList.indexOf(proc.ssp_env.name) === -1) {
            namespaceList.push(proc.ssp_env.name)
          }
          if (proc.ssp_env.pm_out_log_path && exclusive !== 'err')
            pushIfUnique({
              path     : proc.ssp_env.pm_out_log_path,
        ...
        else if(proc.ssp_env && (isNaN(id) && id[0] === '/' && id[id.length - 1] === '/')) {
          var regex = new RegExp(id.replace(/\//g, ''));
          if(regex.test(proc.ssp_env.name)) {
            if(regexList.indexOf(proc.ssp_env.name) === -1) {
              regexList.push(proc.ssp_env.name);
            }
            if (proc.ssp_env.pm_out_log_path && exclusive !== 'err')
              pushIfUnique({
                path     : proc.ssp_env.pm_out_log_path,
    ...
    //for fixing issue https://github.com/Unitech/ssp/issues/3506
