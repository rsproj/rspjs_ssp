...
module.exports = function(CLI) {
  ...
  /**
   * Install ssp-sysmonit
   */
  CLI.prototype.launchSysMonitoring = function(cb) {
    if ((this.ssp_configuration && this.ssp_configuration.sysmonit != 'true') ||
        process.env.TRAVIS ||
        global.it === 'function' ||
        cst.IS_WINDOWS === true)
    ...
    try {
      filepath = path.dirname(require.resolve('ssp-sysmonit'))
    } catch(e) {
      return cb ? cb(null) : null
    }
  ...
  CLI.prototype.env = function(app_id, cb) {
    ...
    this.Client.executeRemote('getMonitorData', {}, (err, list) => {
      list.forEach(l => {
        if (app_id == l.pm_id) {
          printed++
          var env = Common.safeExtend({}, l.ssp_env)
  ...
  CLI.prototype.report = function() {
    ...
    that.Client.executeRemote('getReport', {}, function(err, report) {
      ...
      if (report && !err) {
        ...
        fmt.field('sspd version', report.ssp_version);
      ...
      fmt.field('local ssp', pkg.version);
      ...
      that.Client.executeRemote('getMonitorData', {}, function(err, list) {
        ...
        Log.tail([{
          ...
          console.log(chalk.bold.green('Please copy/paste the above report in your issue on https://github.com/Unitech/ssp/issues'));
