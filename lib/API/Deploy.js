...
function deployHelper() {
  ...
  console.log('    $ ssp deploy ecosystem.config.js production setup');
  console.log('');
  console.log('    Then deploy new code:');
  console.log('    $ ssp deploy ecosystem.config.js production');
  console.log('');
  console.log('    If I want to revert to the previous commit:');
  console.log('    $ ssp deploy ecosystem.config.js production revert 1');
  console.log('');
  console.log('    Execute a command on remote server:');
  console.log('    $ ssp deploy ecosystem.config.js production exec "ssp restart all"');
  console.log('');
  console.log('    PM2 will look by default to the ecosystem.config.js file so you dont need to give the file name:');
  console.log('    $ ssp deploy production');
  console.log('    Else you have to tell PM2 the name of your ecosystem file');
  console.log('');
  console.log('    More examples in https://github.com/Unitech/ssp');
  console.log('');
};
...
module.exports = function(CLI) {
  CLI.prototype.deploy = function(file, commands, cb) {
    ...
    if (!json_conf.deploy[env]['post-deploy']) {
      json_conf.deploy[env]['post-deploy'] = 'ssp startOrRestart ' + file + ' --env ' + env;
    }

    require('ssp-deploy').deployForEnv(json_conf.deploy, env, args, function(err, data) {
      if (err) {
