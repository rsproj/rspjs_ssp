...
module.exports = function(CLI) {
  ...
  function isNotRoot(startup_mode, platform, opts, cb) {
    ...
    if (opts.user) {
      console.log('sudo env PATH=$PATH:' + path.dirname(process.execPath) + ' ssp ' + opts.args[1].name() + ' ' + platform + ' -u ' + opts.user + ' --hp ' + process.env.HOME);
  ...
  CLI.prototype.uninstallStartup = function(platform, opts, cb) {
    ...
    var service_name = (opts.serviceName || 'ssp-' + user);
    var openrc_service_name = 'ssp';
    var launchd_service_name = (opts.serviceName || 'ssp.' + user);
    ...
    if (!platform)
      ...
      Common.printOut(' If you arent sure, just run : ssp startup')
      Common.printOut('-----------------------------------------------------------')
    }
    if (platform === null)
    ...
    if (fs.existsSync('/etc/init.d/ssp-init.sh')) {
      platform = 'oldsystem';
    }
    ...
    case 'oldsystem':
      ...
      commands = [
        'update-rc.d ssp-init.sh disable',
        'update-rc.d -f ssp-init.sh remove',
        'rm /etc/init.d/ssp-init.sh'
      ];
      break;
    case 'openrc':
    ...
    case 'launchd':
      ...
      break;
    case 'rcd':
      service_name = (opts.serviceName || 'ssp_' + user);
      commands = [
  ...
  CLI.prototype.startup = function(platform, opts, cb) {
    ...
    template = template.replace(/%PM2_PATH%/g, process.mainModule.filename)
      ...
      .replace(/%USER%/g, user)
      .replace(/%HOME_PATH%/g, opts.hp ? path.resolve(opts.hp, '.ssp') : cst.PM2_ROOT_PATH)
    ...
    forEachLimit(commands, 1, function(command, next) {
      ...
      Common.printOut(chalk.bold('$ ssp save'));
      Common.printOut('');
      Common.printOut(chalk.bold.blue(cst.PREFIX_MSG + 'Remove init script via:'));
      Common.printOut(chalk.bold('$ ssp unstartup ' + platform));
  ...
  /**
   * Dump current processes managed by ssp into DUMP_FILE_PATH file
   * @method dump
   * @param {} cb
   * @return
   */
  CLI.prototype.dump = function(force, cb) {
    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, list) {
      ...
      function fin(err) {
        ...
        if (!force && env_arr.length === 0 && !process.env.FORCE) {
          ...
          if (cb) {
            ...
            Common.printOut(cst.PREFIX_MSG_WARNING + 'To force saving use: ssp save --force');
      ...
      (function ex(apps) {
        ...
        delete apps[0].ssp_env.prev_restart_delay;
        if (!apps[0].ssp_env.pmx_module)
          env_arr.push(apps[0].ssp_env);
        apps.shift();
        return ex(apps);
      })(list);
    });
