...
function checkIfProcessAreDumped(list) {
  try {
    ...
    var apps_running = list
        .filter(proc => proc.ssp_env.pmx_module != true)
        .map(proc => proc.name)
    var diff = apps_dumped.filter(a => !apps_running.includes(a))
    if (diff.length > 0) {
      Common.warn(`Current process list is not synchronized with saved list. App ${chalk.bold(diff.join(' '))} differs. Type 'ssp save' to synchronize.`)
    }
    else if (apps_dumped.length != apps_running.length) {
      Common.warn(`Current process list is not synchronized with saved list. Type 'ssp save' to synchronize.`)
    }
  } catch(e) {
  }
}
...
function listModulesAndAppsManaged(list, commander) {
  ...
  var id_width = Math.max(
    2 + (Math.max(...list.map((l) => String(l.ssp_env.pm_id || 0).length)) || 0),
    4
  );
  ...
  var sortField = 'name', sortOrder = 'asc', sort,
      fields = {
        ...
        cpu: 'monit.cpu',
        memory: 'monit.memory',
        uptime: 'ssp_env.pm_uptime',
        status: 'ssp_env.status'
      }
  ...
  list.forEach(function(l) {
    ...
    if (l.ssp_env.pm_id > proc_id) {
      proc_id = l.ssp_env.pm_id
    }

    var mode = l.ssp_env.exec_mode
    var status = l.ssp_env.status
    var key = l.ssp_env.pm_id
    key = chalk.bold.cyan(key)

    if (l.ssp_env.axm_options) {
      var is_tracing_enabled = false

      if (l.ssp_env.axm_options.tracing &&
          typeof(l.ssp_env.axm_options.tracing) == 'boolean' &&
          l.ssp_env.axm_options.tracing == true)
        is_tracing_enabled = true

      if (l.ssp_env.axm_options.tracing &&
          l.ssp_env.axm_options.tracing.enabled &&
          typeof(l.ssp_env.axm_options.tracing.enabled) == 'boolean' &&
          l.ssp_env.axm_options.tracing.enabled == true)
        is_tracing_enabled = true

      if (is_tracing_enabled == true)
        l.ssp_env.name = chalk.green('☵')  + ' ' + l.ssp_env.name

      if (l.ssp_env._km_monitored)
        l.ssp_env.name = chalk.bold.green('◉')  + ' ' + l.ssp_env.name
    }

    if (l.ssp_env.pmx_module == true) {
      if (l.ssp_env.name == 'ssp-sysmonit') return
      // ssp ls for Modules
      obj[key] = []
      ...
      // Module version + PID
      if (!CONDENSED_MODE) {
        var pid = l.ssp_env.axm_options.pid ? l.ssp_env.axm_options.pid : l.pid
        obj[key].push(l.ssp_env.version || 'N/A', pid)
      }
      ...
      // Restart
      if (!CONDENSED_MODE)
        obj[key].push(l.ssp_env.restart_time ? l.ssp_env.restart_time : 0)
      ...
      if (!CONDENSED_MODE) {
        ...
        if (l.ssp_env.uid && typeof(l.ssp_env.uid) == 'number') {
          // Resolve user id to username
          let users = Passwd.getUsers()
          Object.keys(users).forEach(function(username) {
            var user = users[username]
            if (user.userId == l.ssp_env.uid) {
              l.ssp_env.uid = user.username
            }
          })
        }
        obj[key].push(chalk.bold(l.ssp_env.uid || l.ssp_env.username))
      }

      UxHelpers.safe_push(module_table, obj)
    }
    else {
      // ssp ls for Applications
      obj[key] = []

      // PM2 ID
      obj[key].push(l.ssp_env.name)

      // Namespace
      if (!CONDENSED_MODE)
        obj[key].push(l.ssp_env.namespace)

      // Version
      if (!CONDENSED_MODE)
        obj[key].push(l.ssp_env.version)
      ...
      // Uptime
      if (!CONDENSED_MODE)
        obj[key].push((l.ssp_env.pm_uptime && status == 'online') ? UxHelpers.timeSince(l.ssp_env.pm_uptime) : 0)
      ...
      // Watch status
      if (!CONDENSED_MODE)
        obj[key].push(l.ssp_env.watch ? chalk.green.bold('enabled') : chalk.grey('disabled'))
...
function miniMonitBar(sys_infos) {
  let sys_metrics = sys_infos.ssp_env.axm_monitor
 ...
 * @method dispAsTable
 * @param {Object} list
 * @param {Object} system informations (via ssp sysmonit/ssp sysinfos)
 */
module.exports = function(list, commander) {
  var ssp_conf = Configuration.getSync('ssp')
  ...
  let sysmonit = list.filter(proc => proc.name == 'ssp-sysmonit')
