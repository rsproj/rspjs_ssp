...
commander.version(pkg.version)
  .description('ssp-dev monitor for any file changes and automatically restart it')
  ...
  .usage('ssp-dev app.js');

var ssp = new PM2.custom({
  ssp_home : path.join(os.homedir ? os.homedir() : (process.env.HOME || process.env.HOMEPATH || process.env.USERPROFILE), '.ssp-dev')
});
...
function run(cmd, opts) {
  ...
  ssp.start(cmd, opts, function(err, procs) {

    if (err) {
      console.error(err);
      ssp.destroy(function() {
        process.exit(0);
      });
      return false;
    }
    ...
    fmt.field('Apps started', procs.map(function(p) { return p.ssp_env.name } ));
    ...
    setTimeout(function() {
      ssp.Client.launchBus(function(err, bus) {
    ...
    Log.devStream(ssp.Client, 'all', opts.raw, timestamp, false);

    process.on('SIGINT', function() {
      console.log('>>>>> [PM2 DEV] Stopping current development session');
      ssp.delete('all', function() {
        ssp.destroy(function() {
          process.exit(0);
        });
      });
...
function autoExit(final) {
  setTimeout(function() {
    ssp.list(function(err, apps) {
      if (err) console.error(err.stack || err);

      var online_count = 0;

      apps.forEach(function(app) {
        if (app.ssp_env.status == cst.ONLINE_STATUS ||
