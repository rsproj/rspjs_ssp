...
commander.version(pkg.version)
  .description('ssp-runtime is a drop-in replacement Node.js binary for containers')
  ...
  .option('--interpreter <interpreter>', 'the interpreter ssp should use for executing app (bash, python...)')
...
var Runtime = {
  ...
  instanciate : function(cmd) {
    this.ssp = new PM2.custom({
      ssp_home : process.env.PM2_HOME || path.join(process.env.HOME, '.ssp'),
    ...
    this.ssp.connect(function(err, ssp_meta) {
  ...
  startLogStreaming : function() {
    if (commander.json === true)
      Log.jsonStream(this.ssp.Client, 'all');
    else if (commander.format === true)
      Log.formatStream(this.ssp.Client, 'all', false, 'YYYY-MM-DD-HH:mm:ssZZ');
    else
      Log.stream(this.ssp.Client, 'all', !commander.formatted, commander.timestamp, true);
  },
  ...
  startApp : function(cmd, cb) {
    function exec() {
      this.ssp.start(cmd, commander, function(err, obj) {
        if (err)
          return cb(err);
        if (obj && obj.length == 0)
        ...
        if (process.env.PM2_RUNTIME_DEBUG)
          Runtime.ssp.disconnect(function() {});
   ...
   */
  exit : function(code) {
    if (!this.ssp) return process.exit(1);

    this.ssp.kill(function() {
      process.exit(code || 0);
    });
  },
  ...
  autoExitWorker : function(fail_count) {
    ...
    var timer = setTimeout(function () {
      Runtime.ssp.list(function (err, apps) {
        if (err) {
          console.error('Could not run ssp list');
        ...
        apps.forEach(function (app) {
          if (!app.ssp_env.pmx_module &&
            (app.ssp_env.status === cst.ONLINE_STATUS ||
              app.ssp_env.status === cst.LAUNCHING_STATUS)) {
            appOnline++;
          }
        });
