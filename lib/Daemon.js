...
var debug        = require('debug')('ssp:daemon');
var pkg          = require('../package.json');
var cst          = require('../constants.js');
var rpc          = require('ssp-axon-rpc');
var axon         = require('ssp-axon');
...
Daemon.prototype.start = function() {
  ...
  d.once('error', function(err) {
    ...
    console.error('[PM2] Resurrecting PM2');
		...
		var path = cst.IS_WINDOWS ? __dirname + '/../bin/ssp' : process.env['_'];
    var fork_new_ssp = require('child_process').spawn('node', [path, 'update'], {
      detached: true,
      windowsHide: true,
      stdio: 'inherit'
    });

    fork_new_ssp.on('close', function() {
...
Daemon.prototype.close = function(opts, cb) {
  ...
  God.bus.emit('ssp:kill', {
    status : 'killed',
    msg    : 'ssp has been killed via CLI'
  });
...
Daemon.prototype.gracefullExit = function() {
  ...
  God.bus.emit('ssp:kill', {
    status : 'killed',
    msg    : 'ssp has been killed by SIGNAL'
  });

  console.log('ssp has been killed by signal, dumping process list before exit...');
  ...
  God.dumpProcessList(function() {
    ...
    eachLimit(processes, 1, function(proc, next) {
      console.log('Deleting process %s', proc.ssp_env.pm_id);
      God.deleteProcessId(proc.ssp_env.pm_id, function() {
        return next();
      });
    }, function(err) {
      try {
...
Daemon.prototype.startLogic = function() {
   ...
   * Attach actions to ssp_env.axm_actions variables (name + options)
   */
  God.bus.on('axm:action', function axmActions(msg) {
    var ssp_env = msg.process;
    var exists  = false;
    var axm_action = msg.data;

    if (!ssp_env || !God.clusters_db[ssp_env.pm_id])
      return console.error('AXM ACTION Unknown id %s', ssp_env.pm_id);

    if (!God.clusters_db[ssp_env.pm_id].ssp_env.axm_actions)
      God.clusters_db[ssp_env.pm_id].ssp_env.axm_actions = [];

    God.clusters_db[ssp_env.pm_id].ssp_env.axm_actions.forEach(function(actions) {
    ...
    if (exists === false) {
      ...
      God.clusters_db[ssp_env.pm_id].ssp_env.axm_actions.push(axm_action);
    }
    msg = null;
  });
  ...
  God.bus.on('axm:option:configuration', function axmMonitor(msg) {
    ...
    try {
      ...
      if (msg.data.name)
        God.clusters_db[msg.process.pm_id].ssp_env.name = msg.data.name;

      Object.keys(msg.data).forEach(function(conf_key) {
        God.clusters_db[msg.process.pm_id].ssp_env.axm_options[conf_key] = Utility.clone(msg.data[conf_key]);
      });
    } catch(e) {
      console.error(e.stack || e);
    }
  ...
  God.bus.on('axm:monitor', function axmMonitor(msg) {
    ...
    Object.assign(God.clusters_db[msg.process.pm_id].ssp_env.axm_monitor, Utility.clone(msg.data));
    msg = null;
  });
