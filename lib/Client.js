...
var debug          = require('debug')('ssp:client');
...
var Client = module.exports = function(opts) {
  ...
  this.ssp_home    = this.conf.PM2_ROOT_PATH;
...
// Init file structure of ssp_home
// This includes
// - ssp pid and log path
...
Client.prototype.launchDaemon = function(opts, cb) {
  ...
  if (!process.env.PM2_DISCRETE_MODE)
    Common.printOut(that.conf.PREFIX_MSG + 'Spawning PM2 daemon with ssp_home=' + this.ssp_home);
  ...
  child.once('message', function(msg) {
     ...
     * it has been already configured before (via ssp link)
     */
    KMDaemon.launchAndInteract(that.conf, {
      machine_name : that.machine_name,
      public_key   : that.public_key,
 ...
 * Once he's connected it trigger the command parsing (on ./bin/ssp file, at the end)
 * @method launchRPC
 * @params {function} [cb]
 * @return
 */
...
Client.prototype.executeRemote = function executeRemote(method, app_conf, fn) {
  ...
  debug('Calling daemon method ssp:%s on rpc socket:%s', method, this.rpc_socket_file);
...
Client.prototype.killDaemon = function killDaemon(fn) {
  ...
  if (process.platform !== 'win32' && process.platform !== 'win64') {
    process.once('SIGQUIT', function() {
      debug('Received SIGQUIT from ssp daemon');
...
Client.prototype.getAllProcessIdWithoutModules = function(cb) {
  ...
  this.executeRemote('getMonitorData', {}, function(err, procs) {
    ...
    var proc_ids = procs
        .filter(proc => !proc.ssp_env.pmx_module)
        .map(proc => proc.pm_id)
...
Client.prototype.getProcessIdByName = function(name, force_all, cb) {
  ...
  this.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    list.forEach(function(proc) {
      if (proc.ssp_env.name == name || proc.ssp_env.pm_exec_path == path.resolve(name)) {
...
Client.prototype.getProcessIdsByNamespace = function(namespace, force_all, cb) {
  ...
  this.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    list.forEach(function(proc) {
      if (proc.ssp_env.namespace == namespace) {
        found_proc.push(proc.pm_id);
        full_details[proc.pm_id] = proc;
      }
    });
...
Client.prototype.getProcessByName = function(name, cb) {
  ...
  this.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    list.forEach(function(proc) {
      if (proc.ssp_env.name == name ||
          proc.ssp_env.pm_exec_path == path.resolve(name)) {
...
Client.prototype.getProcessByNameOrId = function (nameOrId, cb) {
  ...
  this.executeRemote('getMonitorData', {}, function (err, list) {
    ...
    list.forEach(function (proc) {
      if (proc.ssp_env.name === nameOrId ||
        proc.ssp_env.pm_exec_path === path.resolve(nameOrId) ||
        proc.pid === parseInt(nameOrId) ||
        proc.ssp_env.pm_id === parseInt(nameOrId)) {
        foundProc.push(proc);
      }
    });
