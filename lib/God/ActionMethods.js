...
var debug         = require('debug')('ssp:ActionMethod');
...
module.exports = function(God) {
  ...
  God.dumpProcessList = function(cb) {
    ...
    // unless user explicitely did `ssp dump`.
    ...
    function fin(err) {
      ...
      if (process_list.length === 0) {
        ...
        // fix : if no dump file, no process, only module and after ssp update
    ...
    function saveProc(apps) {
      if (!apps[0])
        return fin(null);
      delete apps[0].ssp_env.instances;
      delete apps[0].ssp_env.pm_id;
      // Do not dump modules
      if (!apps[0].ssp_env.pmx_module)
        process_list.push(apps[0].ssp_env);
      apps.shift();
  ...
  God.duplicateProcessId = function(id, cb) {
    ...
    if (!God.clusters_db[id] || !God.clusters_db[id].ssp_env)
      return cb(God.logAndGenerateError('Error when getting proc || proc.ssp_env'), {});

    var proc = Utility.clone(God.clusters_db[id].ssp_env);
  ...
  God.startProcessId = function(id, cb) {
    ...
    if (proc.ssp_env.status == cst.ONLINE_STATUS)
      return cb(God.logAndGenerateError('process already online'), {});
    if (proc.ssp_env.status == cst.LAUNCHING_STATUS)
    ...
    return God.executeApp(God.clusters_db[id].ssp_env, function(err, proc) {
  ...
  God.stopProcessId = function(id, cb) {
    ...
    clearTimeout(proc.ssp_env.restart_task);

    if (proc.ssp_env.status == cst.STOPPED_STATUS) {
      proc.process.pid = 0;
    ...
    console.log('Stopping app:%s id:%s', proc.ssp_env.name, proc.ssp_env.pm_id);
    proc.ssp_env.status = cst.STOPPING_STATUS;

    if (!proc.process.pid) {
      console.error('app=%s id=%d does not have a pid', proc.ssp_env.name, proc.ssp_env.pm_id);
      proc.ssp_env.status = cst.STOPPED_STATUS;
      return cb(null, { error : true, message : 'could not kill process w/o pid'});
    }

    God.killProcess(proc.process.pid, proc.ssp_env, function(err) {
      proc.ssp_env.status = cst.STOPPED_STATUS;
  ...
  God.resetMetaProcessId = function(id, cb) {
    ...
    God.clusters_db[id].ssp_env.created_at = Utility.getDate();
    God.clusters_db[id].ssp_env.unstable_restarts = 0;
    God.clusters_db[id].ssp_env.restart_time = 0;
  ...
  God.restartProcessId = function(opts, cb) {
     ...
     */
    Utility.extend(proc.ssp_env.env, env);
    ...
    if (proc.ssp_env.status === cst.ONLINE_STATUS || proc.ssp_env.status === cst.LAUNCHING_STATUS) {
  ...
  God.restartProcessName = function(name, cb) {
    ...
    eachLimit(processes, cst.CONCURRENT_ACTIONS, function(proc, next) {
      ...
      if (proc.ssp_env.status === cst.ONLINE_STATUS)
        return God.restartProcessId({id:proc.ssp_env.pm_id}, next);
      else if (proc.ssp_env.status !== cst.STOPPING_STATUS
               && proc.ssp_env.status !== cst.LAUNCHING_STATUS)
        return God.startProcessId(proc.ssp_env.pm_id, next);
      else
  ...
  God.stopWatch = function(method, value, fn) {
    ...
    if (method == 'stopAll' || method == 'deleteAll') {
      ...
      processes.forEach(function(proc) {
        God.clusters_db[proc.pm_id].ssp_env.watch = false;
        God.watch.disable(proc.ssp_env);
      });
      ...
      if (env) {
        God.watch.disable(env.ssp_env);
        env.ssp_env.watch = false;
      }
    }
  ...
  God.toggleWatch = function(method, value, fn) {
    ...
    if (env) {
      env.ssp_env.watch = !env.ssp_env.watch;
      if (env.ssp_env.watch)
        God.watch.enable(env.ssp_env);
      else
        God.watch.disable(env.ssp_env);
    }
  ...
  God.startWatch = function(method, value, fn) {
    ...
    if (env) {
      ...
      God.watch.enable(env.ssp_env);
      //env.ssp_env.env.watch = true;
      env.ssp_env.watch = true;
    }
  ...
  God.reloadLogs = function(opts, cb) {
    ...
    processIds.forEach(function (id) {
      ...
      if (cluster && cluster.ssp_env) {
        // Cluster mode
        if (cluster.send && cluster.ssp_env.exec_mode == 'cluster_mode') {
          try {
            cluster.send({
              type:'log:reload'
            });
  ...
  God.sendLineToStdin = function(packet, cb) {
    ...
    if (proc.ssp_env.exec_mode == 'cluster_mode')
  ...
  God.msgProcess = function(cmd, cb) {
    if ('id' in cmd) {
      ...
      proc.ssp_env.axm_actions.forEach(function(action) {
      ...
      if (action_exist == false) {
        return cb(God.logAndGenerateError('Action doesn\'t exist ' + cmd.msg + ' for ' + proc.ssp_env.name), {});
      }
      ...
      (function ex(arr) {
        ...
        var proc_env = God.clusters_db[id].ssp_env;
  ...
  God.monitor = function Monitor(pm_id, cb) {
    if (!God.clusters_db[pm_id] || !God.clusters_db[pm_id].ssp_env)
      return cb(new Error('Unknown pm_id'));

    God.clusters_db[pm_id].ssp_env._km_monitored = true;
  ...
  God.unmonitor = function Monitor(pm_id, cb) {
    ...
    God.clusters_db[pm_id].ssp_env._km_monitored = false;
    return cb(null, { success : true, pm_id : pm_id });
  }
...
function filterBadProcess(pro) {
  if (pro.ssp_env.status !== cst.ONLINE_STATUS) {
    return false;
  }

  if (pro.ssp_env.axm_options && pro.ssp_env.axm_options.pid) {
    if (isNaN(pro.ssp_env.axm_options.pid))  {
      return false;
    }
  }
...
function getProcessId(pro) {
  ...
  if (pro.ssp_env.axm_options && pro.ssp_env.axm_options.pid) {
    pid = pro.ssp_env.axm_options.pid;
  }
