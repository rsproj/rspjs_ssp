...
module.exports = function(God) {
  ...
  God.getFormatedProcess = function getFormatedProcesses(id) {
    if (God.clusters_db[id])
      return {
        ...
        name    : God.clusters_db[id].ssp_env.name,
        ssp_env : God.clusters_db[id].ssp_env,
        pm_id   : God.clusters_db[id].ssp_env.pm_id
      };
    return {};
  };
  ...
  God.getFormatedProcesses = function getFormatedProcesses() {
    ...
    for (var i = 0; i < kl; i++) {
      ...
      if (isNaN(God.clusters_db[key].ssp_env.pm_id)) continue;

      arr.push({
        pid     : God.clusters_db[key].process.pid,
        name    : God.clusters_db[key].ssp_env.name,
        ssp_env : God.clusters_db[key].ssp_env,
        pm_id   : God.clusters_db[key].ssp_env.pm_id
      })
    }
    return arr;
  };
  ...
  God.findByName = function(name) {
    ...
    if (name == 'all') {
      for (var key in db) {
        ...
        if (typeof(God.clusters_db[key].ssp_env.pm_id) === 'number')
          arr.push(db[key]);
      }
      return arr;
    }
    ...
    for (var key in db) {
      if (God.clusters_db[key].ssp_env.name == name ||
          God.clusters_db[key].ssp_env.pm_exec_path == p.resolve(name)) {
        arr.push(db[key]);
      }
    }
    return arr;
   ...
   * @return Literal
   */
  God.processIsDead = function(pid, ssp_env, cb, sigkill) {
    if (!pid) return cb({type : 'param:missing', msg : 'no pid passed'});

    var timeout      = null;
    var kill_timeout = (ssp_env && ssp_env.kill_timeout) ? ssp_env.kill_timeout : cst.KILL_TIMEOUT;
    var mode         = ssp_env.exec_mode;
    ...
    var timer = setInterval(function() {
      ...
      console.log('pid=%d msg=failed to kill - retrying in %dms', pid, ssp_env.kill_retry_time);
      return false;
    }, ssp_env.kill_retry_time);
   ...
   * @method killProcess
   * @param int pid
   * @param Object ssp_env
   * @param function cb
   * @return CallExpression
   */
  God.killProcess = function(pid, ssp_env, cb) {
    if (!pid) return cb({msg : 'no pid passed or null'});

    if (typeof(ssp_env.pm_id) === 'number' &&
        (cst.KILL_USE_MESSAGE || ssp_env.shutdown_with_message == true)) {
      var proc = God.clusters_db[ssp_env.pm_id];
      ...
      if (proc && proc.send) {
        ...
        return God.processIsDead(pid, ssp_env, cb);
      }
      else {
   ...
   * @return
   */
  God.resetState = function(ssp_env) {
    ssp_env.created_at = Date.now();
    ssp_env.unstable_restarts = 0;
    ssp_env.prev_restart_delay = 0;
  };
