...
const debug       = require('debug')('ssp:cli');
...
var KMDaemon    = require('@ssp/agent/src/InteractorClient');
 ...
 *
 * @param {Object}  opts
 * @param {String}  [opts.cwd=<current>]         override ssp cwd for starting scripts
 * @param {String}  [opts.ssp_home=[<paths.js>]] ssp directory for log, pids, socket files
 * @param {Boolean} [opts.independent=false]     unique PM2 instance (random ssp_home)
 * @param {Boolean} [opts.daemon_mode=true]      should be called in the same process or not
 * @param {String}  [opts.public_key=null]       ssp plus bucket public key
 * @param {String}  [opts.secret_key=null]       ssp plus bucket secret key
 * @param {String}  [opts.machine_name=null]     ssp plus instance name
 */
class API {
  ...
  constructor (opts) {
    ...
    this.ssp_home = conf.PM2_ROOT_PATH;
     ...
     * PM2 HOME resolution
     */
    if (opts.ssp_home && opts.independent == true)
      throw new Error('You cannot set a ssp_home and independent instance in same time');

    if (opts.ssp_home) {
      // Override default conf file
      this.ssp_home = opts.ssp_home;
      conf = Object.assign(conf, path_structure(this.ssp_home));
    }
    else if (opts.independent == true && conf.IS_WINDOWS === false) {
      // Create an unique ssp instance
      const crypto = require('crypto');
      var random_file = crypto.randomBytes(8).toString('hex');
      this.ssp_home = path.join('/tmp', random_file);
    ...
    this.ssp_configuration = Configuration.getSync('ssp') || {}
   ...
   *
   * @param {Function} cb callback once ssp is ready for commands
   */
  connect (noDaemon, cb) {
    var that = this;
    ...
    this.Client.start(function(err, meta) {
      ...
      if (meta.new_ssp_instance == false && that.daemon_mode === true)
        return cb(err, meta);

      that.launchSysMonitoring(() => {})
      // If new ssp instance has been popped
      // Launch all modules
  ...
  destroy (cb) {
    ...
    this.killDaemon(function() {
      var cmd = 'rm -rf ' + that.ssp_home;
      var test_path = path.join(that.ssp_home, 'module_conf.json');
      var test_path_2 = path.join(that.ssp_home, 'ssp.pid');

      if (that.ssp_home.indexOf('.ssp') > -1)
        return cb(new Error('Destroy is not a allowed method on .ssp'));

      fs.access(test_path, fs.R_OK, function(err) {
        if (err) return cb(err);
        debug('Deleting temporary folder %s', that.ssp_home);
        sexec(cmd, cb);
      });
    });
  }
   ...
   * Launch modules
   *
   * @param {Function} cb callback once ssp has launched modules
   */
  launchModules (cb) {
...
////////////////////////////
   ...
   *
   * @param {Function} cb callback when ssp has been upgraded
   */
  update (cb) {
    var that = this;

    Common.printOut('Be sure to have the latest version by doing `npm install ssp@latest -g` before doing this procedure.');
  ...
  _startScript (script, opts, cb) {
     ...
     */
    if (app_conf.write) {
      var dst_path = path.join(process.env.PWD || process.cwd(), app_conf.name + '-ssp.json');
    ...
    function restartExistingProcessPathOrStartNew(cb) {
      that.Client.executeRemote('getMonitorData', {}, function(err, procs) {
        ...
        procs.forEach(function(proc) {
          if (proc.ssp_env.pm_exec_path == full_path &&
              proc.ssp_env.name == app_conf.name)
            managed_script = proc;
        });

        if (managed_script &&
          (managed_script.ssp_env.status == conf.STOPPED_STATUS ||
            managed_script.ssp_env.status == conf.STOPPING_STATUS ||
            managed_script.ssp_env.status == conf.ERRORED_STATUS)) {
          // Restart process if stopped
          var app_name = managed_script.ssp_env.name;
        ...
        resolved_paths.env['PM2_HOME'] = that.ssp_home;
    ...
    function startApps(app_name_to_start, cb) {
      ...
      eachLimit(apps_to_start, conf.CONCURRENT_ACTIONS, function(app, next) {
        ...
        that.Client.executeRemote('prepare', resolved_paths, function(err, data) {
          ...
          Common.printOut(conf.PREFIX_MSG + 'App [%s] launched (%d instances)', data[0].ssp_env.name, data.length);
      ...
      eachLimit(ids, concurrent_actions, function(id, next) {
        ...
        that.Client.executeRemote(action_name, opts, function(err, res) {
          ...
          res.forEach(function(proc) {
            Common.printOut(conf.PREFIX_MSG + '[%s](%d) \u2713', proc.ssp_env ? proc.ssp_env.name : process_name, id);

            if (action_name == 'stopProcessId' && proc.ssp_env && proc.ssp_env.cron_restart) {
              Common.warn(`App ${chalk.bold(proc.ssp_env.name)} stopped but CRON RESTART is still UP ${proc.ssp_env.cron_restart}`)
            }

            if (!proc.ssp_env) return false;

            ret.push({
              name         : proc.ssp_env.name,
              namespace: proc.ssp_env.namespace,
    ...
    // operate using regex
      ...
      that.Client.executeRemote('getMonitorData', {}, function(err, list) {
        ...
        list.forEach(function(proc) {
          if (regex.test(proc.ssp_env.name)) {
      ...
      if (that.ssp_configuration.docker == "true" ||
          that.ssp_configuration.docker == true) {
  ...
  speedList (code, apps_acted) {
    ...
    if (apps_acted && apps_acted.length > 0) {
      apps_acted.forEach(proc => {
        acted.push(proc.ssp_env ? proc.ssp_env.pm_id : proc.pm_id)
      })
    }
    ...
    function doList(err, list) {
      ...
      if (process.stdout.isTTY === false) {
        ...
        if (that.gl_interact_infos) {
          var dashboard_url = `https://app.ssp.io/#/r/${that.gl_interact_infos.public_key}`
        ...
        //Common.printOut(chalk.white.italic(' Use `ssp show <id|name>` to get more details about an app'));
      }
      ...
      // if (process.stdout.isTTY) if looking for start logs
        ...
        // setTimeout(() => {
        //   Common.info(`Log streaming exited automatically, run 'ssp logs' to continue watching logs`)
        //   return that.exitCli(code ? code : conf.SUCCESS_EXIT);
        // }, 10000)
  ...
  scale (app_name, number, cb) {
    ...
    function addProcs(proc, value, cb) {
      (function ex(proc, number) {
        ...
        that.Client.executeRemote('duplicateProcessId', proc.ssp_env.pm_id, ex.bind(this, proc, number));
      })(proc, number);
    }
    ...
    function rmProcs(procs, value, cb) {
      ...
      (function ex(procs, number) {
        if (number++ === 0) return cb();
        that._operate('deleteProcessId', procs[i++].ssp_env.pm_id, ex.bind(this, procs, number));
      })(procs, number);
    }
  ...
  describe (ssp_id, cb) {
    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, list) {
      ...
      list.forEach(function(proc) {
        if ((!isNaN(ssp_id)    && proc.pm_id == ssp_id) ||
          (typeof(ssp_id) === 'string' && proc.name  == ssp_id)) {
          found_proc.push(proc);
        }
      });

      if (found_proc.length === 0) {
        Common.printError(conf.PREFIX_MSG_WARNING + '%s doesn\'t exist', ssp_id);
  ...
  deepUpdate (cb) {
    ...
    var child = sexec("npm i -g ssp@latest; ssp update");
...
require('./API/ssp-plus/link.js')(API);
require('./API/ssp-plus/process-selector.js')(API);
require('./API/ssp-plus/helpers.js')(API);
