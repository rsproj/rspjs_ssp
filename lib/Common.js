...
Common.determineSilentCLI = function() {
  // ssp should ignore -s --silent -v if they are after '--'
...
Common.prepareAppConf = function(opts, app) {
  ...
  /**
   * Do not copy internal ssp environment variables if acting on process
...
Common.verifyConfs = function(appConfs) {
  ...
  for (var i = 0; i < appConfs.length; i++) {
    ...
    /**
     * If command is like ssp start "python xx.py --ok"
     ...
     * comes from ssp --uid <> --gid <> or --user
     */
    if (app.uid || app.gid || app.user) {
      ...
      if (process.env.NODE_ENV != 'test' && process.getuid && process.getuid() !== 0) {
        Common.printError(cst.PREFIX_MSG_ERR + 'To use --uid and --gid please run ssp as root');
...
Common.getCurrentUsername = function(){
  ...
  if (os.userInfo) {
    try {
      ...
      // https://github.com/Unitech/ssp/issues/3184
    }
  }
