...
unknown_os ()
{
    ...
    echo
    echo "For more informations, please read the documentation on http://ssp.io/"
    exit 1
}
...
finalize_yum_repo ()
{
    ...
    yum install -y pygpgme --disablerepo='Keymetrics_ssp'
    ...
    if [ "$?" != "0" ]; then
        ...
        sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/Keymetrics_ssp.repo
    fi

    echo "Installing yum-utils..."
    yum install -y yum-utils --disablerepo='Keymetrics_ssp'
    ...
    echo "Generating yum cache for Keymetrics_ssp..."
    yum -q makecache -y --disablerepo='*' --enablerepo='Keymetrics_ssp'
}

finalize_zypper_repo ()
{
    zypper --gpg-auto-import-keys refresh Keymetrics_ssp
}
...
install_ssp ()
{
    ...
    $PKG_MANAGER install -y ssp 2> /dev/null
    ...
    if [ "$CURR_USER" == "root" ] || [ "$CURR_USER" == "" ]; then
        ...
        echo "         user in the ssp group using the following"
        echo "         command:      usermod -aG ssp <username>"
        echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    else
        echo -n "Adding $CURR_USER to group ssp..."
        usermod -aG ssp $CURR_USER
        echo "done."
    fi
}
...
main ()
{
    ...
    yum_repo_config_url="https://packagecloud.io/install/repositories/$REPOSITORY_OWNER/ssp/config_file.repo?os=${os}&dist=${dist}&source=script"

    if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
        yum_repo_path=/etc/zypp/repos.d/Keymetrics_ssp.repo
    else
        yum_repo_path=/etc/yum.repos.d/Keymetrics_ssp.repo
        install_node
    fi
    ...
    echo "You now need to logout of your system and login again in order to be able to use the 'ssp' command."
}
